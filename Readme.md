...
Anyway, it is recommended to use `pre-commit`, which will run `clang-format-10` automatically.
...
Anyway, you should see no difference, as the repo is already formatted. Instead, it is recommended to run `pre-commit` automatically before committing in a local repo, installing git hooks with the following command:
