...
class Controller(object, metaclass=abc.ABCMeta):
    ...
    def scale_action(self, action):
        ...
        if self.action_scale is None:
            self.action_scale = abs(self.output_max -
                                    self.output_min) / abs(self.input_max -
                                                           self.input_min)
    ...
    def update(self, force=False):
        ...
        if self.new_update or force:
            ...
            self.ee_pos = np.array(self.sim.data.site_xpos[self.sim.model.site_name2id(
                self.eef_name)])
            ...
            self.force = np.clip(self.sim.data.sensordata, 0, 10000)
