...
if __name__ == "__main__":
    ...
    p = PandaArm(model_path=MODEL_PATH+'panda_block_table.xml',
            render=False, compensate_gravity=False, smooth_ft_sensor=True)
    tau_vect = np.linspace(0.01, 0.1, num=10) #convertito in stiffness da la stiffness falsa
    xdata = []
    ydata = []
    ...
    for tau_index in range(tau_vect.shape[0]):
        ...
        p.model.geom_solref[17,0] = tau_vect[tau_index]
        ...
        while i < target_traj.shape[0]:
            ...
            if i == 10: # activate force control when the robot is near the table
                ctrl.change_ft_dir([1,1,1,1,1,1]) # start force control along Z axis
                ctrl.set_goal(target_traj[i, :],
                            target_ori, goal_force=[0, 0, -98.1], goal_torque=[0, 0, 0]) # target force in cartesian frame
            else:
        ...
        # print("tau =", contact.solimp[0], "damprati" , contact.solimp[1])
        # print(contact.solref[0], contact.solref[1])
    ...
    plt.show()
    plt.savefig('K falsa vs penetration_ cnst force.png')
