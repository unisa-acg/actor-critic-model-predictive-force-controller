...
class MujocoContactValidation:
    ...
    def get_inv_AR(self, sim, efc_addresses):
        ...
        Args:
            ...
            efc_addresses (NDArray): vector that stores the indeces of the contact 
            points
        ...
        inv_AR[:max_efc + 1, :max_efc + 1] = inv_AR_efc[:max_efc + 1, :max_efc + 1]
    ...
    def get_delta_a(self, sim):
        ...
        a_ref = np.zeros(njmax, dtype=np.float64)    
        c_array = np.zeros(6, dtype=np.float64)         
        efc_addresses = np.zeros(ncon, dtype=np.int32) 
        normal_forces = np.zeros(ncon, dtype=np.float64) 

        combinations_forces = np.zeros(len(self.body_combinations),
                                            dtype=np.float64)
        combinations_forces_built_in = np.zeros(len(self.body_combinations),
                                                dtype=np.float64)

        if self.combinations_forces.shape[0] != len(
                combinations_forces):
            self.combinations_forces = np.zeros(
                len(combinations_forces), dtype=np.float64)
            self.combinations_forces = np.transpose(
        ...
        a_ref = [-(k_vect[k] * dist_njmax[k] + 
                    b_vect[k] * vel_njmax[k]) for k in range(len(k_vect))]
        # for k in range(len(k_vect)):
        #     a_ref[k] = -(k_vect[k] * dist_njmax[k] +
        ...
        if ncon != 0:
            ...
            for ind_efc in efc_addresses:
                ...
                combinations_forces_built_in[ind_tuple] = (
                    combinations_forces_built_in[ind_tuple] +
                    normal_forces[ind_efc])

            self.combinations_forces[
                :, self.contact_forces_calls] = combinations_forces

            # Store the forces with mujoco built-in method for comparison
            self.combinations_forces_built_in[
                :, self.contact_forces_calls] = combinations_forces_built_in
    ...
    def contact_forces_to_csv(self, sim, csv_name):
        ...
        # TODO: instead of geom 1 and geom 0 transform the names into the real names of 
        # the bodies

        if ncon != 0:
            #with open("contact_data_simulation.csv", "a") as f:
            with open("../output/{}_{}.csv".format(csv_name, 
