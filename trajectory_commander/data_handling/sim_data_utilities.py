...
class SimulationDataUtilities:
    ...
    * plot_ee_pos_vel(): plot the position and velocity of the robot end-effector.
    ...
    def __init__(self, steps):
        self.ncalls = 0
        self.forces_vect = np.zeros(
            (6,steps), dtype=np.float64, order="C"
        )
        self.dist = np.zeros(
            (1,steps), dtype=np.float64, order="C"
        )
        self.vel = np.zeros(
            (1,steps), dtype=np.float64, order="C"
        )
    ...
    def get_info_robot(self, env, bodies_name):
        ...
        self.sensor_data[self.ncalls_get_info_robot,:] = env.sim.data.sensordata  #check that they are equal to ee_force and torque
        ...
        self.ee_pos[self.ncalls_get_info_robot,:] = env.sim.data.get_body_xpos(bodies_name)
        #ee_quat = env._get_observations()['robot0_eef_quat']
        self.ee_quat[self.ncalls_get_info_robot,:] = env.sim.data.get_body_xquat(bodies_name)
        #ee_velp = env.sim.data.get_site_xvelp('gripper0_grip_site')
        self.ee_vel[self.ncalls_get_info_robot,:] = env.sim.data.get_body_xvelp(bodies_name)
        #ee_velp2 = env.sim.data.get_site_xvelp('gripper0_ft_frame')
        #ee_velr = env.sim.data.get_site_xvelp('gripper0_grip_site')
        self.ee_omg[self.ncalls_get_info_robot,:] = env.sim.data.get_body_xvelr(bodies_name)
        #ee_ori = quat2euler(ee_quat)
        #ee_ori = ee_quat
        self.ee_ori[self.ncalls_get_info_robot,:] = quat_utils.quat2euler(env.sim.data.get_body_xquat(bodies_name))
        ...
        J_pos = np.array(
            env.sim.data.get_site_jacp(grip_site).reshape((3, -1))[:,
                                                                joint_indexes])
        J_ori = np.array(
            env.sim.data.get_site_jacr(grip_site).reshape((3, -1))[:,
                                                                joint_indexes])
    ...
    def plot_torques(self):
        ...
        for j in range(2):
            for k in range(3):
                axs[j, k].plot(step_vect, self.actuated_joints[0:self.ncalls_get_info_robot, n_graph])
    ...
    def plot_ee_pos_vel(self):
        ...
        axs[0].grid()
        axs[1].plot(step_vect, self.ee_pos[0:self.ncalls_get_info_robot, 1], 'tab:orange')
        axs[1].set_title('$Ee \; pos_y$')
        axs[1].grid()
        axs[2].plot(self.ee_pos[0:self.ncalls_get_info_robot, 0], self.ee_pos[0:self.ncalls_get_info_robot, 1], 'tab:red')
        ...
        axs[0].grid()
        axs[1].plot(step_vect, self.ee_vel[0:self.ncalls_get_info_robot, 1], 'tab:orange')
